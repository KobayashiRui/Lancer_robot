...
#define step1_1 18 //ステッピングモータ(右)の速度決定のピン番号
#define step1_2 19 //ステッピングモータ(右)の回転方向決定のピン番号
...
void control_stepper_motor(float velo){
  ...
  digitalWrite(step1_1, HIGH);
  digitalWrite(step2_1, HIGH);
  delayMicroseconds(delay_num);
  digitalWrite(step1_1, LOW);
  digitalWrite(step2_1, LOW);
  }
  }
}
