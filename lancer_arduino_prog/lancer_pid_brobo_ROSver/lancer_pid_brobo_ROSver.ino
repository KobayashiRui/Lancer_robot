...
float setPointOld = 0;
float target_angle;
float throttle;//コントローラーはこれを操作する
float steering;//コントローラーはこれを操作する
...
ros::Subscriber<std_msgs::Int8MultiArray> sub("contl_data", &messageCb);
//===============================================================================
...
void setup()
{
  ...
  //pinMode(4, OUTPUT); // ENABLE MOTORS => not use
  /*deforut setting=================================
  ...
  */
  ...
  pinMode(5, OUTPUT); // DIR MOTOR 2  PORTC,6
  //digitalWrite(4, HIGH);  // Disbale motors => not use
...
*/
  ...
  //digitalWrite(4, LOW);   // Enable stepper drivers => not use
  ...
  for (uint8_t k = 0; k < 5; k++)
  {
    setMotorSpeedM1(5);
    setMotorSpeedM2(5);
    //BROBOT.moveServo1(SERVO_AUX_NEUTRO + 100); => not use servo
    delay(200);
    setMotorSpeedM1(-5);
    setMotorSpeedM2(-5);
    //BROBOT.moveServo1(SERVO_AUX_NEUTRO - 100); => not use aervo
    delay(200);
  }
  //BROBOT.moveServo1(SERVO_AUX_NEUTRO);
  ...
  // OSC initialization osc => not use
  //OSC.fadder1 = 0.5;
  //OSC.fadder2 = 0.5;
